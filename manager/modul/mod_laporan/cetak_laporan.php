...
if (empty($dt1) && empty($dt2)) {
    ...
    $query = mysqli_query($conn, "SELECT * FROM td_pesanan JOIN tm_user ON td_pesanan.nik = tm_user.nik WHERE td_pesanan.status ='Transaksi Sudah Selesai' OR td_pesanan.status = 'Sudah Terkonfirmasi'") or die(mysqli_error($conn));
}
