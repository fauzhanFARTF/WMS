...
$query = mysqli_query($conn, "SELECT * FROM td_pesanan JOIN tm_user ON td_pesanan.nik = tm_user.nik JOIN tm_unit ON td_pesanan.id_unit = tm_unit.id_unit WHERE td_pesanan.status ='Ditolak' ORDER BY td_pesanan.kd_transaksi DESC") or die(mysqli_error($conn));
    ...
    <div class="col-md-12">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                Manajemen Pesanan Kontrakan yang Ditolak
            </div>
            <hr>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <?php
                $uery = mysqli_query($conn, "SELECT * FROM tm_unit");
                ...
                <form action="..\admin\modul\mod_pesanan\add_pesanan.php" class="form-horizontal" method="POST" onsubmit="return validasi_input(this)">
                    <div class="form-group">
                        <label class="control-label col-sm-4">Pilih Kontrakan :</label>
                        <div class="col-sm-6">
                            <select class="form-control" name="id_unit" id="unit">
                                <option value="pilih">--Pilih Kontrakan--</option>
                                <?php while ($kol = mysqli_fetch_array($uery)) { ?>
                                    <option value="<?php echo $kol['id_unit'] ?>"><?php echo $kol["title"] ?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
...
<!-- <script type="text/javascript">
    function validasi_input(form) {
        if (form.id_unit.value == "pilih") {
            alert("Anda belum memilih unit!");
            return (false);
        }
        return (true);
    }
